...
class Excel_Daily {
	...
	public static function generate( string $date, string $type = 'sm', bool $is_camp = false ): ?string {
		require_once MEAL_MENU_DIR . 'vendor/autoload.php';

		$db   = DB::instance();
		$dept_info = $db->get_department( $type );
		$data_type = $type;
		if ( ! $is_camp && $dept_info && ! empty( $dept_info['merged_with'] ) ) {
			$target = $db->get_department( $dept_info['merged_with'] );
			if ( $target ) {
				$data_type = $target['code'];
			}
		}
		$cal  = $is_camp ? $db->get_camp_calendar_day( $date, $data_type ) : $db->get_calendar_day( $date, $data_type );
		if ( ! $cal || ! $cal['template_id'] ) {
			return null;
		}

		$tpl      = $is_camp ? $db->get_camp_template( (int) $cal['template_id'] ) : $db->get_template( (int) $cal['template_id'] );
		$items    = $is_camp ? $db->get_camp_template_items( (int) $cal['template_id'] ) : $db->get_template_items( (int) $cal['template_id'] );
