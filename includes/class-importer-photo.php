...
class Importer_Photo {
	...
	private static function build_prompt(): string {
		return 'Ты — ассистент по анализу школьных меню. Проанализируй фотографию и извлеки данные.

Правила:
- meal_type: "breakfast" (завтрак), "breakfast2" (завтрак 2), "lunch" (обед), "afternoon_snack" (полдник), "dinner" (ужин), "dinner2" (ужин 2)
- section: категория блюда (гор.блюдо, суп, напиток, закуска, гарнир, салат, выпечка, фрукты, каша и т.д.)
- dish_name: название блюда — обязательно, если названия нет — пропусти строку
- Если вес/БЖУ/ккал отсутствуют на фото — ставь null
- Если номер рецептуры (ТТК) не указан — пустая строка
- Каждое блюдо — отдельный элемент в массиве items
- Различай "завтрак" и "завтрак 2" — это разные meal_type
- Отсортируй items по порядку приёма пищи: сначала завтрак, потом обед, полдник, ужин
